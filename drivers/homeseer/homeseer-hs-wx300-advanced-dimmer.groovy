...
@Field static final Map<Short,Map> zwaveParameters = [
   ...
   4: [size: 1, input: [name: "param.4", title: "Paddle orientation",
       type: "enum", options: [[0: "Top of paddle turns load on (DEFAULT)"],[2:"Bottom of paddle turns load on"]]]],
...
metadata {
   definition (name: "HomeSeer HS-WX300 Advanced (Dimmer)", namespace: "RMoRobert", author: "Robert Morris", importUrl: "https://raw.githubusercontent.com/RMoRobert/Hubitat/master/drivers/homeseer/homeseer-hs-wx300-advanced-dimmer.groovy") {
      ...
      fingerprint mfr: "000C", prod: "4447", deviceId: "4036", inClusters: "0x5E,0x55,0x9F,0x6C"  // WX300 (dimmer, S2)
      fingerprint mfr: "000C", prod: "4447", deviceId: "4036", inClusters: "0x5E,0x85,0x59,0x5A,0x7A,0x87,0x72,0x8E,0x73,0x9F,0x6C,0x55,0x86,0x26,0x70,0x5B"  // WX300 (dimmer)
   }
...
List<String> setStatusLED(String ledNumber, String colorName, String blink) {
   ...
   //cmds.add(secure(zwave.configurationV1.configurationSet(parameterNumber: ledNum + 20, scaledConfigurationValue: colorNum, size: 1)))
   cmds.add(cmd)
