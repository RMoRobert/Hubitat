 ...
 * 
 *  Version History
 *  2022-09-29: BETA - additiional start/stopPositionChange() tweaks for firmware 3.14+
 *  2022-09-03: start/stopPositionChange() tweaks for firwmare 3.14+
...
@Field static final Map<Short,Map> zwaveParameters = [
   ...
   11: [input: [name: "param.11", type: "enum", title: "startLevelChange behavior", options: [[0: "Use full range"],[1: "Open to 50% (default)"]]], size: 1]
]

@Field static ConcurrentHashMap<Long, ConcurrentHashMap<Short, String>> supervisedPackets = [:]  as ConcurrentHashMap 
@Field static ConcurrentHashMap<Long, Short> sessionIDs = [:] as ConcurrentHashMap
...
metadata {
   definition (name: "iBlinds v3 (Community Driver)", namespace: "RMoRobert", author: "Robert Morris", importUrl: "https://raw.githubusercontent.com/RMoRobert/Hubitat/master/drivers/iBlinds-v3.groovy") {
      ...
      command "logParemeterValue", [[name:"parameterNumber*", type: "NUMBER", description: "Z-Wave parameter number"]]
   ...
   preferences {
      ...
      input name: "levelChangeRate", type: "enum", description: "", title: "levelChange rate",
         options: [[1: "1 second"],[2: "2 seconds"],[3: "3 seconds (DEFAULT)"],[4: "4 seconds"], [5: "5 seconds"], [6: "6 seconds"], [7: "7 seconds"],
                  [10: "10 seconds"], [15:"15 seconds"]], defaultValue: 3
...
void zwaveEvent(hubitat.zwave.commands.supervisionv1.SupervisionReport cmd) {
   ...
   if (!supervisedPackets[device.idAsLong]) { supervisedPackets[device.idAsLong] = [:] as ConcurrentHashMap }
...
List<String> startPositionChange(String direction) {
   ...
   //hubitat.zwave.Command cmd = zwave.switchMultilevelV2.switchMultilevelStartLevelChange(dimmingDuration: 3, upDown: openClose, ignoreStartLevel: true, startLevel: 0)
   hubitat.zwave.Command cmd = zwave.switchMultilevelV1.switchMultilevelStartLevelChange(upDown: openClose, ignoreStartLevel: true, startLevel: 0)
...
List<String> startPositionChange(String direction) {
   if (enableDebug) log.debug "startPositionChange($direction)"
   if (direction == "open") return setLevel(99, (levelChangeRate as Short) ?: 3)
   else return setLevel(1, (levelChangeRate as Short) ?: 3)
}

List<String> stopPositionChange() {
   if (enableDebug) log.debug "stopPositionChange()"
   hubitat.zwave.Command cmd = zwave.switchMultilevelV2.switchMultilevelStopLevelChange()
   return [zwaveSecureEncap(cmd)]
}
...
List<String> setLevel(Number value, Number duration) {
   ...
   hubitat.zwave.Command cmd = zwave.switchMultilevelV2.switchMultilevelSet(value: level, dimmingDuration: dimmingDuration as Short)
   return [zwaveSecureEncap(supervisedEncap(cmd))]
}
...
String logParemeterValue(Number parameterNumber) {
   log.debug "logParemeterValue($parameterNumber)"
   if (!enableDebug) log.warn "Enable debug logging to see result of configurationGet"
