 ...
 *
 * Changelog:
 * 4.0    (2022-01-16)  - Allows creation of DBC 4.0 child apps; removed "presets" (use Hubitat app cloning instead)
   ...
   page(name: "mainPage", title: "Dimmer Button Controller", install: true, uninstall: true) {
      if (app.getInstallationState() == "INCOMPLETE") {
         ...
         section("Dimmer Button Controller Child Apps") {
               app(name: "childApps4", appName: "Dimmer Button Controller (Child App) 4", namespace: "RMoRobert", title: "Add new Dimmer Button Controller automation", multiple: true)
               // Show DBC 3.x child (and allow new creation) if any instances already exist:
               if (app?.getChildApps()?.find { it.name == "Dimmer Button Controller (Child App) 3" }) {
                  app(name: "childApps3", appName: "Dimmer Button Controller (Child App) 3", namespace: "RMoRobert",
                     title: "Add new Dimmer Button Controller 3.x automation (deprecated)", multiple: true)
               }
